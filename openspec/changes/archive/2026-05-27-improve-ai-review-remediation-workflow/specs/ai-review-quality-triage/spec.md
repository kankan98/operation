...
including generated sections, validation results, confidence, source
references, human review state, feedback signals, and downstream eligibility.

#### Scenario: Run has blocking validation
- **WHEN** a selected AI review run has failed or blocked validation results
- **THEN** the quality triage summary SHALL mark the run as blocked, prioritize
  validation repair, provide remediation-safe route data, and SHALL NOT mark
  downstream reuse as ready
...
#### Scenario: Triage can feed remediation without new data access
- **WHEN** the remediation workflow derives an action plan from quality triage
- **THEN** the triage data SHALL provide deterministic priority keys, repair
  route labels, affected section context, and downstream state using only the
  selected protected run detail response
...
The project SHALL provide repeatable local verification for quality triage and
remediation inputs without calling live DeepSeek or any external provider by
default.
