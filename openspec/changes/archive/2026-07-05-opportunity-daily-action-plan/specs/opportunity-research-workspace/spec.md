...
- **THEN** the response SHALL include workflow action items with stable identifiers, labels, priority, counts, reasons, suggested filter state, generated timestamp, and a non-scoring caveat
...
- **WHEN** action plan counts change because workflow metadata changes
...
- **THEN** the UI SHALL show daily action items with labels, counts, and workflow reasons near the review summary
