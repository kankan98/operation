# ai-review-run-persistence Specification

## Purpose
Define the local-only AI review run persistence slice for tenant/team-scoped
input snapshots, knowledge snapshots, prompt metadata, provider invocation
metadata, structured outputs, validation results, human decisions, feedback
signals, downstream artifact references, authorization, sensitive data gates,
and rollback verification before real provider execution or public save flows
are implemented.
## Requirements
...
- **THEN** it can identify that AI review run local persistence, provider port, and server-only generation orchestrator exist locally, while RAG, queue, public API, Server Action, UI save flows, and production AI release remain out of scope
