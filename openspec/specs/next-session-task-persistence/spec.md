# next-session-task-persistence Specification

## Purpose
Define the local-only next-session task persistence runtime for source-linked
task records, ownership, lifecycle transitions, checklist and dependency
readiness, review closure, feedback signals, tenant/team scope, and rollback
verification before public task CRUD or AI downstream task creation is exposed.
## Requirements
