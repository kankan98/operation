...
The AI review run contract SHALL identify which parts of the AI review run boundary are implemented as local persistence, which provider-facing boundary is implemented as a local adapter, and which full workflow capabilities remain deferred.

#### Scenario: Contract is updated after local persistence
- **WHEN** the AI review run persistence slice is completed
- **THEN** `docs/contracts/ai-review-run.md` identifies local schema/repository/check scripts, provider port, and generation orchestrator as implemented while still marking public APIs, Server Actions, RAG, queueing, UI save flows, and production AI release as not implemented
...
- **THEN** `docs/contracts/ai-review-run.md` identifies the server-only `AiProviderPort`, DeepSeek adapter, environment parser, and local verifier as implemented while still marking full AI review prompt orchestration, RAG, queues, public APIs, Server Actions, UI save flows, and production AI release as not implemented
