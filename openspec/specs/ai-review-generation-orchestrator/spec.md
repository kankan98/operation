# ai-review-generation-orchestrator Specification

## Purpose
Define the server-only AI review generation orchestrator for bounded reviewed
snapshots, prompt fingerprinting, structured AI review output schema
validation, provider error mapping, local product validation, and fake-provider
verification before public APIs, Server Actions, UI save flows, RAG, queues, or
production AI release are implemented.
