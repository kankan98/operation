...
## Purpose
Define the local V0 browser workflow for `/ai-review`, including authenticated
team context resolution, review-ready session selection, bounded run
preparation, local fake-provider execution by default, gated live-model mode,
human section decisions, feedback signals, downstream draft references, safe
error handling, and repeatable verification.
## Requirements
