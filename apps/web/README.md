...
- `/sessions` operator V0 直播场次采集工作流、`/ai-review` operator V0
  AI 复盘工作流、`/rackets` 静态球拍产品库和 `/knowledge` 静态学习中枢。
...
- `/ai-review` operator V0 浏览器工作流：可进入本地 V0 团队上下文，加载已提交场次，
  ...
  默认不调用真实 DeepSeek，不接 RAG，也不自动创建话术资产或下场任务。
...
- 面向用户的完整业务 CRUD、Server Action 保存流程、生产数据库 provider、连接池、备份或恢复；
  ...
  受保护 API runtime，`/sessions` 和 `/ai-review` 已有 operator V0 浏览器闭环，其他业务域仍没有浏览器可操作保存流程。
- AI 复盘生产模型发布、Server Action、RAG 上下文选择、分析任务和正式下游创建流程；当前 AI 复盘
...
除 `/sessions` 和 `/ai-review` 已升级为 operator V0 浏览器工作流，`/rackets`、`/knowledge` 已升级为静态工作台外，其余工作流页面
...
调用 AI 或生成运营建议。当前已有本地-only 知识生命周期 schema、server-only
repository 和 local-only 受保护 Route Handler runtime，验证来源登记、手动 claim、团队笔记、
审核队列、审核决策、冲突阻断、冲突解决、发布 readiness、no-store 响应和 tenant/team 隔离；
后续真正开放浏览器保存或公开知识库时，需要新增 OpenSpec 变更定义表单状态、薄
Server Action 或 fetch wrapper、刷新任务、AI grounding、RAG snapshot 和反馈评估。
...
查看校验结果，并对区块记录采纳或暂不用。它复用现有 auth cookie/session runtime、
本地 V0 bootstrap、受保护 AI review API runtime 和 server-only execution service；默认不会调用真实
DeepSeek，也不会接 RAG、公开来源发现、队列、Server Action 或自动下游创建。现有 auth cookie
...
operator/team session、`capture_session` / `run_ai_review` V0 权限、安全 session view、no-store
响应、敏感元数据脱敏和幂等 seed 行为。它用于本地 `/sessions` 与 `/ai-review` 浏览器闭环，
不是生产登录 provider、邀请流程或团队管理 UI。
...
审核、冲突阻断/解决、发布、跨团队隔离、no-store 响应和敏感元数据脱敏。它不会创建
浏览器保存 UI、公开来源抓取、RAG snapshot 或 AI 调用。
