...
- Internal trial MVP hardening：`/trial` ready 后的继续动作、`/` cockpit、六个已实现工作面
  可达性、试用恢复状态和公网预览边界由 `trial-mvp:check` 与 Playwright 共同验证；它仍不是生产登录。
...
- 生产账号登录、auth provider、公开登录路由、邀请、团队管理 UI 和生产认证服务。
...
受保护 Route Handlers；默认 auth cookie 仍是 `Secure`。HTTP 公网 IP 预览只有在显式开启
internal V0 preview cookie policy 后才可跑完整 V0 认证浏览器流，且不能录入真实客户、订单、私信、
供应商、定价策略或完整未脱敏转录。
这些工作面现在也通过 Next.js Proxy 做 public trial route gate：没有 app-owned session cookie
时先进入 `/trial`，有 cookie 时页面可渲染，但所有数据仍必须通过受保护 API 做服务端授权。
...
现有 auth cookie 默认要求 `Secure` cookie；HTTP 公网 IP 预览需要显式 internal V0 preview cookie
policy 才能跑完整保存闭环。正式公开试用仍应引入 HTTPS 和生产登录。
...
调用 RAG grounding 或开放完整公开 CRUD。HTTP 公网 IP 预览需要显式 internal V0 preview cookie policy
才可跑认证流。
...
“开始处理”和检查项完成/阻塞。它不会创建通知、日历、导出、跨团队分派或自动关闭任务。HTTP
公网 IP 预览需要显式 internal V0 preview cookie policy 才可跑认证流。
...
`auth:cookie-check` 会创建临时 auth cookie fixture，验证 `Set-Cookie` 安全属性、显式 internal V0
preview cookie policy、request cookie
...
`trial-mvp:check` 会验证内部试用 MVP 路径：public trial route gate、unsafe next fallback、
V0 bootstrap/session/logout、六个已实现工作面的受保护 list API 在同一个 scoped session 下可访问、
logout 后 protected API 被拒绝，以及敏感元数据不外泄。它用于可用版本收口，不替代各领域业务检查。

`public-trial-auth:check` 会验证 public trial route gate：缺 session cookie 的工作面跳转到
`/trial?next=...`、带 cookie 放行、`/` / `/trial` / API / 静态资源不被错误跳转、unsafe next
回退、redirect no-store header、cookie 名称一致性和敏感元数据不外泄。它不是生产登录 provider
或最终授权检查；业务数据仍由受保护 Route Handlers 校验。
...
预览模式会为 V0 内部演示团队签发短期 non-`Secure`、`HttpOnly`、`SameSite=Lax` cookie，只用于
演示和内部评估。不要在 HTTP 预览里录入真实客户、订单、私信、供应商、定价策略或完整未脱敏转录。
关闭 `OPERATION_ALLOW_INSECURE_V0_PREVIEW_COOKIE` 即回到 secure-by-default cookie 行为。
