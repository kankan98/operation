...
- `/sessions` operator V0 直播场次采集工作流、`/ai-review` operator V0
  AI 复盘工作流、`/talk-tracks` operator V0 话术资产工作流、`/next-actions`
  operator V0 下场任务工作流、`/rackets` 静态球拍产品库和 `/knowledge` 静态学习中枢。
...
- 面向用户的完整业务 CRUD、Server Action 保存流程、生产数据库 provider、连接池、备份或恢复；
  ...
  受保护 API runtime，`/sessions`、`/ai-review`、`/talk-tracks` 和 `/next-actions`
  已有 operator V0 浏览器闭环，产品库和知识库仍没有浏览器可操作保存流程。
...
`/sessions`、`/ai-review`、`/talk-tracks` 和 `/next-actions` 已升级为 operator V0 浏览器工作流。
`/rackets`、`/knowledge` 仍是静态工作台，不读取、不保存、不生成真实业务数据，也不调用 AI 或外部平台。
