...
export async function GET(request: Request): Promise<Response> {
  if (!readAuthSessionReferenceFromRequestCookie(request)) {
    return handleTalkTrackAssetsListRoute(null, null, request);
  }

  const { client, db } = createDatabaseConnection();

  try {
    return await handleTalkTrackAssetsListRoute(
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    return handleTalkTrackAssetsCreateRoute(null, null, request);
  }

  const { client, db } = createDatabaseConnection();

  try {
    return await handleTalkTrackAssetsCreateRoute(
      createAuthSessionRepository(db),
      createTalkTrackAssetRepository(db),
      request,
    );
