...
type AiReviewDownstreamRouteContext = {
  params: Promise<{
    runId?: string;
  }>;
};
...
export async function POST(
  request: Request,
  context: AiReviewDownstreamRouteContext,
): Promise<Response> {
  ...
  if (
    ...
    return handleAiReviewDownstreamArtifactRoute(null, null, request, params);
  }

  const { client, db } = createDatabaseConnection();

  try {
    return await handleAiReviewDownstreamArtifactRoute(
