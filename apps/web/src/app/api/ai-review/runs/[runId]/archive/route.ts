...
type AiReviewArchiveRouteContext = {
  params: Promise<{
    runId?: string;
  }>;
};
...
export async function POST(
  request: Request,
  context: AiReviewArchiveRouteContext,
): Promise<Response> {
  ...
  if (
    ...
    return handleAiReviewRunArchiveRoute(null, null, request, params);
  }

  const { client, db } = createDatabaseConnection();

  try {
    return await handleAiReviewRunArchiveRoute(
