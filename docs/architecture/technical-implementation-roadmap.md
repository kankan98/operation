...
- 当前代码：`apps/web` 使用 pnpm、Next.js App Router、TypeScript、React、Tailwind CSS、
  ...
  AI review execution service、AI review protected API runtime、
...
| Auth | `AuthPort` + app-owned tenant/team/membership/session ledger + provider-neutral guard + server-only cookie/request bridge + local auth Route Handlers + gated operator V0 bootstrap + unified internal trial access + public trial route gate + explicit internal V0 HTTP preview cookie policy + production access/HTTPS transport gate planning | 已接受边界，本地 guard、session resolver、cookie runtime、`GET /api/auth/session`、CSRF-checked `POST /api/auth/logout`、local-only `POST /api/auth/operator-v0-session`、统一内部试用入口、`/trial` public trial entry、Next.js Proxy route gate、显式 internal V0 preview cookie policy 和生产访问/HTTPS 实施门禁拆解已部分实现，provider 延后 | 避免 provider SDK 泄漏到业务层，先支持可撤销/可过期 session 到 `AuthContext` 的服务端映射，并给未来 protected Route Handler / Server Action 预留 session/logout HTTP 边界；V0 bootstrap、统一 trial access、public trial route gate 和 preview cookie policy 只用于试用/内部工作流验证，最终授权仍在服务端 Route Handler/repository；生产访问/HTTPS 门禁用于约束下一轮 provider/login/团队入口/CSRF/origin/secure cookie/HTTPS runtime | 下一轮可按门禁比较并实现登录 provider、公开登录路由、team switching、邀请或 V1-Lite 单团队入口、HTTPS 域名和 secure cookie 生产策略；真实敏感数据治理仍需后续阶段 9 门禁 |
| Trial MVP hardening | `/trial` continue flow + `/` cockpit + six-workbench scoped API smoke via `trial-mvp:check` | 已接受，本地与预览验证已接入 | 已把现有 V0 工作台收口成可评估版本，验证 route gate、session scope、logout denial、safe redaction 和 public preview 边界 | 仍不替代生产登录、HTTPS、备份、监控或真实敏感数据治理 |
...
| Deployment | Docker preview 已接受，生产 provider 延后；生产访问/HTTPS transport gate 已拆解域名、TLS、HTTP redirect、secure cookie origin 和 preview/production split | 当前只作预览；可显式启用 internal V0 HTTP preview cookie policy 做内部评估；生产 HTTPS 仍未实现 | 服务器预览可用，但不是生产架构；HTTP preview cookie policy 只服务短期 V0 内部试用；下一轮 HTTPS runtime 必须从域名/TLS/secure cookie/回滚门禁开始 | 域名、SSL、备份、监控、生产登录和真实敏感数据入口后再选生产 |
...
`implement-ai-review-api-runtime` 已加入 prompt metadata、run prepare/list/detail/execute、
decision/feedback/downstream/archive local-only 受保护 Route Handler、mutation CSRF、safe JSON、
fake-provider execution、no-store 响应、tenant/team scope 和本地 PostgreSQL 回滚式 smoke check。
...
`/api/talk-tracks/**` 话术资产工作流、`/api/next-actions/**` 下场任务工作流和
`/api/ai-review/**` AI 复盘工作流受保护 Route Handler 外，公开 UI、其他业务 Route Handler、Server Action、AI/RAG snapshot、
...
4. AI 复盘 run 持久化与生成：run、输入快照、知识快照、prompt/provider 元数据、输出、校验、人工审核、
   ...
   orchestrator、server-only execution service 和 protected API runtime 已本地部分实现；后续补 RAG snapshot、
   Server Action、UI 保存、队列和生产发布。
...
- 当前已本地实现 AI review protected API runtime：受保护 prompt metadata、run prepare/list/detail、
