...
状态：规划基线，AI provider port 已本地部分实现。

本文件固定未来 Q&A Agent、AI 复盘、RAG 和反馈学习的首选架构。当前项目已有
server-only `AiProviderPort` / DeepSeek adapter / `ai-provider:check` 本地验证，以及
server-only AI review generation orchestrator / `ai-review:generation-check` fake-provider 验证，
server-only AI review execution service / `ai-review:execution-check` fake-provider 验证；
仍未把 LLM 接入公开 API、浏览器保存流程或 Q&A 工作流，不创建向量索引，不搜索公开网页，也不实现对外后端接口；
本地开发数据库中已经有数据基础、核心运营 repository 和 AI review run ledger 的 local-only 验证切片。

全局阶段路线见 `docs/architecture/technical-implementation-roadmap.md`。如果未来实现
AI、RAG、来源发现、队列、对象存储或外部集成，必须先确认对应阶段、契约和 OpenSpec
仍然正确。
...
| 领域编排 | 项目自有 Agent orchestrator，显式状态机；AI 复盘已有本地 generation orchestrator 和 execution service 切片 | `AgentOrchestrator` 接口 |
| LLM | 用户指定 DeepSeek 已作为首个本地 provider gate；OpenAI Responses API 保留为已研究参考方向 | `AiProviderPort` |
...
4. 用户已指定 DeepSeek API、base URL 和 `deepseek-v4-pro`；当前已通过 `AiProviderPort`
   provider gate 落地 server-only adapter、环境变量密钥、结构化输出校验、失败模式、日志脱敏和
   本地回滚验证。AI 复盘已有 server-only generation orchestrator 和 execution service，可用已脱敏输入快照和已审核知识快照
   经 provider port 生成可审核结构化建议，并把 provider metadata、输出、校验和失败状态写回本地 run ledger；
   后续仍需单独 OpenSpec 才能接公开 API、UI 保存、RAG、队列或 Q&A。
   OpenAI Responses API 保留为结构化输出和可追踪元数据的参考实现方向。
...
- AgentOrchestrator：控制状态机、调用检索、调用 LLM、组装结构化输出；AI 复盘本地 generation
  orchestrator 已覆盖 prompt 构建、provider 调用、输出校验和失败映射，execution service 已覆盖
  run ledger、provider metadata、输出、校验和 review-ready / failure 状态持久化的最小切片。
- AiProviderPort：隐藏 DeepSeek、OpenAI 或其它 provider 的 SDK/API 细节。
...
- 模型直接改知识库。
- 未审核网页搜索结果直接成为答案依据。
- UI 组件直接调用 DeepSeek、OpenAI、AI SDK、向量库或数据库。
...
- Provider：DeepSeek（用户指定，已通过 OpenSpec provider gate 本地落地）。
- Base URL：`https://api.deepseek.com`。
- Model：`deepseek-v4-pro`。
- API：DeepSeek chat completions，经 `AiProviderPort` adapter 调用。
- 输出：结构化 schema，先校验再展示或保存。
- 工具：当前不启用；未来只通过 adapter 开启，并按 OpenSpec 定义允许范围。
- 错误：拒绝、超时、限流、schema mismatch、部分输出都进入 non-success run state。
- AI 复盘 generation：当前仅 server-only fake-provider 验证，不开放浏览器触发，不默认 live smoke；
  成功结果仍是待审核建议，不能自动发布到知识、话术或任务。

Provider 替换要求：

- 新 provider 也必须通过 `AiProviderPort`。
- API key 只能通过环境变量或正式 secret 管理配置，不能提交、记录到日志或写入 OpenSpec。
...
- DeepSeek official API docs: base URL、模型、请求/错误语义和未来 provider gate。
- OpenAI platform docs: Responses API、tools、structured outputs，作为参考实现方向。
