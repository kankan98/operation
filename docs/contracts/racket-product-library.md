...
Status: draft
Runtime: partially implemented, local-only product, alias, source, review, and publish repository
...
和 `rackets:source-review-check` 回滚式验证。

当前仍没有公开 UI 保存流程、Route Handler、Server Action、公开来源导入/发现 provider、
AI 调用、RAG snapshot、生产数据库 provider 或公网预览数据保存能力。
...
- 公开 API、Server Action、UI 表单保存、批量导入、公开来源发现/导入 provider、搜索服务、
  AI/RAG snapshot 或生产持久化。
...
- `CreateRacketProductCommand`：本地 repository 层已部分实现产品和别名创建，不包含 API、
  Server Action 或公开保存流程。
...
- `ListRacketProductsQuery`：本地 repository 层已部分实现 tenant/team scoped list，不包含
  公开 API、全文搜索或复杂筛选。
