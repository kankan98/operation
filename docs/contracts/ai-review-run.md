...
- `ai-provider:check` 默认只用 fake fetch 验证，不消耗 provider 额度；只有显式设置
  `DEEPSEEK_API_KEY` 且 `DEEPSEEK_LIVE_SMOKE=1` 或 `AI_REVIEW_LIVE_SMOKE=1` 时才允许最小
  live smoke。
- AI review live-model gate 已本地落地：`OPERATION_ENABLE_LIVE_AI_REVIEW=1` 是真实模型
  浏览器/API 执行的必要 release switch，单独存在 `DEEPSEEK_API_KEY` 不会启用真实模型；
  `GET /api/ai-review/live-model/status` 只返回 safe readiness，不暴露密钥、prompt、provider
  payload、cookie 或数据库连接串；`ai-review:live-gate-check` 默认不调用真实 DeepSeek。
...
- `ai-review:execution-check` 默认只用 fake provider 验证，不消耗 provider 额度，不读取真实
  ...
  prompt/secret 泄漏。
- AI review protected API runtime 已本地落地为 Route Handler 边界：覆盖 prompt version metadata、
  run prepare/list/detail/execute、human decision、feedback signal、downstream artifact reference
  和 archive；所有 mutation 需要 `x-operation-csrf: ai-review`，所有请求需要显式 tenant/team scope，
...
- Live gate 测试：`pnpm ai-review:live-gate-check` 覆盖 disabled gate、missing config、configured
  readiness、auth/scope failure、safe live execute disabled response 和 no sensitive leakage；默认不调用
  真实 DeepSeek。
...
- Route Handler 测试：`pnpm ai-review:route-check` 覆盖 prompt metadata、run prepare/list/detail、
  fake-provider execute、review decision、feedback、downstream reference、archive、缺 cookie、缺 CSRF、
  缺 scope、cross-team isolation、provider failure、no-store、redaction 和 transaction rollback。
