...
受保护 AI 复盘 API runtime / `ai-review:route-check` 回滚式验证；仍没有 Server Action、RAG
检索、队列、UI 保存流程、生产 AI 发布或自动任务创建。
...
- 后续 RAG snapshot、重试策略、队列、Server Action、UI 保存、生产 AI 发布和自动下游创建仍必须
  单独创建 OpenSpec。
