...
PostgreSQL/Drizzle schema、server-only repository 和 `next-actions:check`
回滚式验证；没有任何对外可调用接口、Route Handler、Server Action、浏览器保存流程、
AI provider 调用、队列、通知、日历同步、导出或真实生产任务行为。
...
| 阶段 4 | 手动创建、编辑、指派、状态更新、列表筛选、审核关闭、反馈信号和归档；当前仅 repository/check 本地验证 | 从静态页面状态反推数据库结构，或在没有真实登录边界时开放公开 CRUD |
...
- 首个 runtime 切片应该从 AI 复盘采纳创建任务、人工场次跟进、知识缺口处理，还是团队负责人任务看板开始。
