...
3. V0.9 trial readiness cockpit：`/trial` 与 `/` 的可用版本已接入六个已实现工作面的 scoped list API、
   ...
   HTTPS 域名、备份恢复、敏感数据治理、AI/RAG 评测和观测脱敏的真实阻断项与下一波建议；不直接引入生产登录、
   RAG、公开来源发现或外部平台集成。
...
1. V1 生产访问与 HTTPS 门禁：以当前 V1 生产门禁 workflow 为输入，优先把生产登录 provider/公开登录路由、
   team switching、邀请、HTTPS 域名和真实数据进入条件作为一个 workflow 级 OpenSpec 波次比较和接受；
   不把 public trial route gate 当成生产认证，也不在没有 HTTPS 和真实数据边界时开放正式试用。
