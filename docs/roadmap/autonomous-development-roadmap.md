...
- V0.9 试用就绪 cockpit 已接入试用运行证据：`/trial` 和 `/` 在 verified trial session 后把六个工作面的
  ...
  组合为内部试用阶段，明确区分继续收集、先修卡点、V0.9 可试用和生产门禁规划。当前 V0 验收包波次
  正在把这些证据收口成内部发布结论、证据强度、阻断项、下一步动作和生产门禁说明；它不等于生产登录、
  HTTPS、备份、敏感数据治理、RAG/Q&A 或生产监控。
...
3. V0.9 trial readiness cockpit：`/trial` 与 `/` 的可用版本已接入六个已实现工作面的 scoped list API、
   反馈证据、试用运行证据、阶段判断、下一步动作和六步 checklist。当前重点是完成 V0 验收包归档前验证、
   Playwright 桌面/移动检查和 Docker 预览部署，让内部评估人员能看到发布结论、证据强度、卡点和生产门禁；
   不引入生产登录、RAG、公开来源发现或外部平台集成。
4. V0 trial feedback and run evidence review：已收集的 V0 反馈和本次试用运行会继续转成 scoped 证据摘要、
   主要热点、代表备注、步骤卡点和下一步建议，并作为 V0.9 readiness 的输入，帮助团队判断下一轮应优先做
   体验打磨、示例数据、AI 质量、来源信任、下游承接、生产准备，还是继续收集反馈；不引入外部 analytics
   或生产反馈系统。
