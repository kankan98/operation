...
- V0 试用反馈收集已补齐，证据复盘正在推进：`/trial` 和 `/` cockpit 在 verified trial session 后
  提供结构化反馈入口，通过本地-only `v0_trial_feedback` 记录评估角色、工作面、评分、问题类型、
  简短备注和真实工作可用性；当前波次把 scoped 反馈汇总为证据摘要、热点和下一步建议，用作后续
  V0/V1 排序依据；它不接外部 telemetry、analytics SDK 或第三方问卷。
- V0.9 试用就绪 cockpit 正在收口：`/trial` 和 `/` 在 verified trial session 后把六个工作面的
  protected readiness、scoped feedback evidence、下一步动作和六步试用 checklist 组合为内部试用阶段，
  明确区分继续收集、先修卡点、V0.9 可试用和生产门禁规划；它不等于生产登录、HTTPS、备份、
  敏感数据治理、RAG/Q&A 或生产监控。
...
- `/ai-review` operator V0 浏览器工作流已部分实现：可进入本地 V0 团队上下文、加载已提交场次、
  ...
  feedback signals 聚合为运行级 cockpit 与区块级复核提示，帮助运营先判断可用性、证据缺口和下游门槛。
...
3. V0.9 trial readiness cockpit：当前优先把 `/trial` 与 `/` 的可用版本收口成“可以交给内部人员试”的
   cockpit，在 verified trial session 后汇总六个已实现工作面的 scoped list API、反馈证据、阶段判断、
   下一步动作和六步试用 checklist。本轮目标是让评估人员知道先跑哪条路径、试完反馈什么、哪些卡点阻碍
   真实使用，以及是否只能进入生产门禁规划；不引入生产登录、RAG、公开来源发现或外部平台集成。
4. V0 trial feedback evidence review：已收集的 V0 反馈会继续转成 scoped 证据摘要、主要热点、
   代表备注和下一步建议，并作为 V0.9 readiness 的输入，帮助团队判断下一轮应优先做体验打磨、
   示例数据、AI 质量、来源信任、下游承接、生产准备，还是继续收集反馈；不引入外部 analytics
   或生产反馈系统。
...
1. V0.9 试用证据运行：本轮 readiness cockpit 归档和 Docker 预览部署后，用演示/脱敏数据跑 2-3 条完整路径，
   汇总 cockpit 阶段、操作卡点、文案误解、移动端溢出、AI 复盘质量、来源信任和下游话术/任务转化问题，
   再决定下一个 workflow 级提案；不要再重复建设基础 demo seed。
...
4. AI 复盘 MVP：server-only generation orchestrator、execution service、受保护 API runtime、
   ...
   source refs 和 validation results 汇总为运营可扫读的可信度提示，用作后续评测、知识复核和提示词复核输入；
