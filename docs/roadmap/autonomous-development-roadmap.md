...
- 面向用户的登录后真实持久化、完整受保护 CRUD、API/Server Action 保存流程、生产数据库 provider、
  连接池、备份和恢复；当前只有产品库 create/list 具备 local-only 受保护 Route Handler 验证，
  其余直播场次、知识生命周期、AI 复盘 run、话术资产和下场任务持久化仍仅限本地 repository 验证。
