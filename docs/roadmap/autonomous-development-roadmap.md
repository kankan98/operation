...
- V0.9 试用就绪 cockpit 已接入试用运行证据：`/trial` 和 `/` 在 verified trial session 后把六个工作面的
  ...
  组合为内部试用阶段，明确区分继续收集、先修卡点、V0.9 可试用和生产门禁规划；它不等于生产登录、
...
3. V0.9 trial readiness cockpit：`/trial` 与 `/` 的可用版本已接入六个已实现工作面的 scoped list API、
   反馈证据、试用运行证据、阶段判断、下一步动作和六步 checklist。当前重点是完成归档前验证、
   Playwright 桌面/移动检查和 Docker 预览部署，让内部评估人员能按路径试用并记录卡点；不引入生产登录、
   RAG、公开来源发现或外部平台集成。
