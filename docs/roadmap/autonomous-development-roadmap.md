...
- 本地-only operator V0 bootstrap 已部分实现：`POST /api/auth/operator-v0-session` 在显式启用和
  CSRF header 下创建内部演示 operator/team session，用于本地 `/sessions`、`/ai-review`、
  `/talk-tracks` 和 `/next-actions` 浏览器工作流；它不是生产登录 provider。
...
- 面向用户的登录后真实持久化、完整受保护 CRUD、Server Action 保存流程、生产数据库 provider、
  ...
  受保护 Route Handler 验证，`/sessions`、`/ai-review`、`/talk-tracks` 和 `/next-actions`
  具备 operator V0 浏览器闭环；产品库和知识库仍没有浏览器保存流程。
- AI 复盘生产模型发布、Server Action、RAG 上下文选择、Q&A 模型调用和自动发布/自动完成的正式下游流程；当前已有
  server-only DeepSeek provider adapter、AI 复盘 generation orchestrator、execution service、受保护
  API runtime 和 `/ai-review` 到下游工作台的本地 V0 fake-provider 浏览器工作流验证。
...
3. AI 复盘 MVP：server-only generation orchestrator、execution service、受保护 API runtime、
   `/ai-review` operator V0 浏览器 workflow 以及 AI 复盘到话术/任务的下游 V0 浏览器闭环已完成本地
   fake-provider 方向；下一步按 `ai-review-run` 契约推进生产模型发布策略、评测样例、RAG snapshot、
   Server Action wrapper 或队列前置设计，
