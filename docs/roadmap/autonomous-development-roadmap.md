...
| `/rackets` | 静态球拍产品库工作台 |
| `/knowledge` | 静态知识库学习中枢 |
| `/ai-review` | Operator V0 AI 复盘工作流 |
| `/talk-tracks` | 话术资产占位页 |
...
- 面向用户的登录后真实持久化、完整受保护 CRUD、Server Action 保存流程、生产数据库 provider、
  ...
  受保护 Route Handler 验证，`/sessions` 和 `/ai-review` 具备 operator V0 浏览器闭环；其他业务域仍没有浏览器保存流程。
- AI 复盘生产模型发布、Server Action、RAG 上下文选择、Q&A 模型调用和正式下游创建流程；当前已有
  server-only DeepSeek provider adapter、AI 复盘 generation orchestrator、execution service、受保护
  API runtime 和 `/ai-review` 本地 V0 fake-provider 浏览器工作流验证。
...
11. AI 复盘 run：`ai-review-run` 契约已起草，且本地-only run、输入快照、知识快照、
    ...
    受保护 API runtime 和 `/ai-review` operator V0 浏览器 workflow 已落地；后续实现 RAG snapshot、
    Server Action、生产模型发布、队列、正式下游创建或评测时必须从该契约开始，并同步更新偏离处。
...
3. AI 复盘 MVP：server-only generation orchestrator、execution service、受保护 API runtime 和
   `/ai-review` operator V0 浏览器 workflow 已完成本地 fake-provider 方向；下一步按
   `ai-review-run` 契约推进生产模型发布策略、评测样例、RAG snapshot、Server Action wrapper 或队列前置设计，
   面向用户的公开试用仍需先解决 HTTPS/provider login、公开登录路由、team switching 和受保护 mutation 边界。
