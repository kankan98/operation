...
- 面向用户的登录后真实持久化、完整受保护 CRUD、Server Action 保存流程、生产数据库 provider、
  ...
  受保护 Route Handler 验证，AI 复盘也具备 local-only 受保护 API runtime，但仍没有浏览器保存流程。
...
9. 直播场次保存：`session-capture` 契约已起草，且本地-only 场次、主播职责、商品顺序、
   ...
   create/list/detail/autosave/submit 受保护 Route Handler 已部分实现；后续实现浏览器保存 UI、
   Server Action wrapper、转录导入或 AI 复盘输入时必须从该契约开始，并同步更新偏离处。
