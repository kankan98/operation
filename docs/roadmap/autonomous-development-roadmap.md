...
- V0 试用反馈收集正在补齐：`/trial` 和 `/` cockpit 在 verified trial session 后提供结构化反馈入口，
  通过本地-only `v0_trial_feedback` 记录评估角色、工作面、评分、问题类型、简短备注和真实工作可用性，
  用作后续 V0/V1 排序依据；它不接外部 telemetry、analytics SDK 或第三方问卷。
...
3. V0 usable trial workflow：当前优先完成 `/trial` 与 `/` 的可用版本收口，在 verified trial
   session 后汇总六个已实现工作面的 scoped list API，显示 V0 进度、每步记录数和下一步建议。
   这把内部可试用 V0 从“多个可访问页面”推进到“可按运营路径演示和收集反馈”的版本；不引入生产
   登录、RAG、公开来源发现或外部平台集成。
4. `/rackets` 产品库工作台：已能进入 V0 团队上下文、创建/加载 scoped 产品草稿、登记来源、
   提交审核、审核来源/产品、发布产品，并显示审核队列和下游准备；下一步是产品编辑、版本化
   snapshot、公开来源发现或与知识版本/RAG 的连接。
5. Agent 架构基线：按 `AiProviderPort`、`RetrievalPort`、PostgreSQL + pgvector、
...
6. 技术实施阶段路线和技术蓝图基线：已建立；后续 runtime work 必须从阶段路线、技术状态、
   项目自有 port/adapter 和对应契约开始。
7. 认证与团队边界：`auth-team-tenant` 契约已起草，且本地-only 授权守卫基础、app-owned
   ...
   并同步更新偏离处。
8. 数据基础：`data-foundation` 契约已起草，且本地-only runtime 已部分实现；后续
   workflow-specific schema、repository、事务、幂等和审计字段必须在认证/团队契约约束下设计
   tenant/team ownership，并同步更新偏离处。
9. 球拍产品库持久化/API：`racket-product-library` 契约已起草，且本地-only 产品、别名、来源、
...
10. 直播场次保存：`session-capture` 契约已起草，且本地-only 场次、主播职责、商品顺序、
   ...
   必须从该契约开始，并同步更新偏离处。
11. 知识库持久化：`knowledge-lifecycle` 契约已起草，且本地-only 来源登记、claim、团队笔记、
...
12. AI 复盘 run：`ai-review-run` 契约已起草，且本地-only run、输入快照、知识快照、
...
13. Q&A Agent answer：`qa-agent-answer` 契约已起草，后续实现 provider 调用、RAG 检索、
    答案持久化、引用、反馈、缺失知识、web discovery 审核流或评测时必须从该契约开始，
    并同步更新偏离处。
14. 话术资产：`talk-track-asset` 契约已起草，且本地-only 资产、版本、场景、区块、来源引用、
...
15. 下场任务：`next-session-task` 契约已起草，且本地-only 任务、来源证据、负责人、检查项、
...
1. V0 试用反馈证据复盘：本轮反馈收集归档后，先用演示/脱敏数据跑 2-3 条完整路径，汇总操作卡点、
   文案误解、移动端溢出、AI 复盘质量、来源信任和下游话术/任务转化问题，再决定下一个 workflow 级提案。
2. 认证 provider/login 入口：本地 guard、session ledger、request cookie resolver、logout
   invalidation、safe session route、CSRF-checked logout route、统一内部试用入口和 public trial
   route gate 已完成；下一步按 `auth-team-tenant` 契约比较生产 provider/login、HTTPS 域名、
   team switching 和 invitation，而不能把 public trial route gate 当成生产认证。
3. AI provider gate：用户已提供 DeepSeek base URL 和 `deepseek-v4-pro`，且本地
   `AiProviderPort` / DeepSeek adapter 已完成；不要把 key 写入仓库，live smoke 只能显式启用。
4. AI 复盘 MVP：server-only generation orchestrator、execution service、受保护 API runtime、
   `/ai-review` operator V0 浏览器 workflow 以及 AI 复盘到话术/任务的下游 V0 浏览器闭环已完成本地
   fake-provider 方向；当前反馈学习波次把采纳、拒绝、缺知识、来源不准、证据弱和下游使用记录为
   可审计质量信号，用作后续评测、知识复核和提示词复核输入；下一步按 `ai-review-run` 契约推进
...
5. 反馈学习：基础浏览器闭环先记录接受、编辑、拒绝、再生成、缺知识、来源不准、证据弱和下游使用等信号；
   后续再建设正式评测集、知识复核队列、提示词复核和 Q&A/RAG 质量分析。
6. Q&A Agent 第一阶段：按 `qa-agent-answer` 契约只基于已审核知识和团队知识回答，
