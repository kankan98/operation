...
6. 认证与团队边界：`auth-team-tenant` 契约已起草，且本地-only 授权守卫基础、app-owned
   session runtime 和 server-only cookie/request runtime 已部分实现；后续实现登录、公开登录/登出路由、
   middleware、团队、角色、
...
1. 认证 provider/login 入口：本地 guard、session ledger、request cookie resolver 和 logout
   invalidation 已完成；下一步按 `auth-team-tenant` 契约选择 provider 或实现最小公开登录/登出路由、
   middleware / route-level protection，不能只靠本地 guard/session/cookie runtime 或前端隐藏控件。
...
3. AI 复盘 MVP：server-only generation orchestrator 和 execution service 已完成本地 fake-provider 验证；下一步按
   ...
   面向用户的公开 CRUD 仍需先解决 provider/login、公开登录/登出路由和受保护 mutation 边界。
