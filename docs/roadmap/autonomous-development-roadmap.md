...
4. 查证不确定项：优先用项目文档、已安装 skill、官方文档、公开来源和可靠行业资料。
5. 更新 OpenSpec：把目标、取舍、风险、验收和任务写清楚。
6. 写契约草案：如果后续要接后端、数据库、AI、RAG 或外部集成，先定义领域对象、
   输入输出、状态机、错误、权限、敏感数据和验证。
7. 实现最小闭环：优先做一个能验证价值的能力切片。
8. 验证并部署：运行相关检查，必要时更新公网容器。
9. 沉淀路线：把新发现的缺口、下一步、协作需求和文档更新写回 Goal、路线或规格。
...
2. 接口契约基线：为球拍产品、直播场次、知识生命周期、AI run 和 Q&A Agent 写契约草案。
...
5. 认证与团队边界：登录、团队、角色、租户隔离、受保护路由。
6. 数据基础：PostgreSQL、Drizzle schema、迁移、Zod 校验、审计字段。
7. 球拍产品库持久化：先补 `racket-product-library` 契约，再实现型号、别名、规格、
   卖点、适用人群、价格带和审核状态。
8. 直播场次保存：手动录入、草稿恢复、长文本边界、问题和异议结构化。
9. 知识库持久化：来源登记、公开数据元信息、审核状态、版本和刷新时间。
