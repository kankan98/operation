...
3. V0 usable trial workflow：当前优先完成 `/trial` 与 `/` 的可用版本收口，在 verified trial
   session 后汇总六个已实现工作面的 scoped list API，显示 V0 进度、每步记录数、已加载演示场景和
   下一步建议。本轮 demo-data 收口把内部可试用 V0 从“多个可访问页面”推进到“可直接按运营路径演示、
   收集反馈”的版本；不引入生产登录、RAG、公开来源发现或外部平台集成。
4. V0 trial feedback evidence review：当前波次优先把已收集的 V0 反馈转成 scoped 证据摘要、
   主要热点、代表备注和下一步建议，帮助团队判断下一轮应优先做体验打磨、示例数据、AI 质量、
   来源信任、下游承接、生产准备，还是继续收集反馈；不引入外部 analytics 或生产反馈系统。
...
1. V0 试用反馈证据复盘：本轮 demo-data 归档和 Docker 预览部署后，先用演示/脱敏数据跑 2-3 条完整路径，
   汇总操作卡点、文案误解、移动端溢出、AI 复盘质量、来源信任和下游话术/任务转化问题，再决定下一个
   workflow 级提案；不要再重复建设基础 demo seed。
