...
- 本地-only operator V0 session workflow：`POST /api/auth/operator-v0-session` 在显式启用和
  ...
  V0 context 进入本地 AI 复盘闭环。它仍不是生产登录
...
- 面向用户的登录后完整 CRUD、Server Action、生产数据库 provider、连接池、备份和恢复；
  ...
  `/sessions` 和 `/ai-review` 具备 operator V0 浏览器闭环；其他业务域仍没有浏览器可操作保存流程。
- AI 复盘生产模型发布、Server Action、RAG、Q&A 模型调用、Web discovery、公开审核队列 UI；当前已有
  ...
  API runtime 和 `/ai-review` 本地 V0 fake-provider 浏览器 workflow 验证。
...
4. AI 复盘输入快照、知识快照、输出、失败状态、人工审核、server-only generation orchestrator、
   ...
   已进入本地验证阶段；下一步若接 RAG snapshot、生产 provider 发布、Server Action、队列、
   公开试用登录或自动下游创建，
...
4. `ai-review-run` 契约草案已建立，且本地-only run、输入快照、知识快照、prompt 版本元数据、
   ...
   `ai-review:v0-check` 已落地；后续 RAG snapshot、Server Action、生产模型发布、
   话术/任务下游真实创建、队列或评测必须先读取并更新 `docs/contracts/ai-review-run.md`。
...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    ...
    `/sessions` 到 `/ai-review` 的 V0 浏览器闭环；更合理的候选是阶段 2 provider/login 选择、公开登录路由、
    middleware/team switching、产品库 UI 保存入口/编辑/API 扩展、AI 复盘评测、RAG 前置切片或队列设计。
