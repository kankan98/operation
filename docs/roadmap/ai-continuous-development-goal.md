...
1. 球拍产品库已进入本地 V0 浏览器 source/review/publish 阶段，产品、别名、来源、审核和发布门禁
...
6. `auth-team-tenant` 契约草案已建立，且本地-only 授权守卫基础、app-owned session runtime、
   server-only cookie/request runtime、session/logout auth route runtime 和统一内部试用入口已部分实现；后续认证
...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    ...
    middleware/team switching、产品库编辑、知识库刷新/版本回滚、AI 复盘评测、RAG 前置切片或队列设计。
