...
- 面向用户的登录后完整 CRUD、Server Action、生产数据库 provider、连接池、备份和恢复；
  当前产品库 create/list 与直播场次 create/list/detail/autosave/submit 具备 local-only 受保护
  Route Handler 验证，其余知识生命周期、AI 复盘 run、话术资产和下场任务持久化仍仅限本地
  repository 验证，没有公开保存流程。
