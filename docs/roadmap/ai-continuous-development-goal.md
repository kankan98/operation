...
- 面向用户的登录后完整 CRUD、Server Action、生产数据库 provider、连接池、备份和恢复；
  当前产品库 create/list、直播场次 create/list/detail/autosave/submit 和知识生命周期
  source/claim/note/review/conflict/publish 具备 local-only 受保护 Route Handler 验证，AI 复盘 run、
  话术资产和下场任务持久化仍仅限本地 repository 验证，没有公开保存流程。
- AI 复盘公开触发/API/UI 保存、RAG、Q&A 模型调用、Web discovery、公开审核队列 UI；当前只有
...
4. AI 复盘输入快照、知识快照、输出、失败状态、人工审核、server-only generation orchestrator
   和 server-only execution service 已进入本地验证阶段；下一步若接 RAG snapshot、公开 API、浏览器保存、队列、生产 provider 发布或真实 UI 触发，
   必须先更新 `docs/contracts/ai-review-run.md` 并创建 provider/runtime OpenSpec。
5. 话术资产和下场任务闭环；话术资产和下场任务均已具备本地 repository 验证，后续公开保存、
   AI 下游候选或团队看板仍需真实认证和 API/Server Action 边界。
...
2. `session-capture` 契约草案已建立，且本地-only 场次、主播职责、商品顺序、笔记、
   ...
   受保护 Route Handler 已部分实现；后续浏览器保存 UI、Server Action wrapper、转录导入、
   AI 复盘输入或外部平台同步必须先读取并更新 `docs/contracts/session-capture.md`。
...
4. `ai-review-run` 契约草案已建立，且本地-only run、输入快照、知识快照、prompt 版本元数据、
   ...
   `ai-provider:check`、`ai-review:generation-check` 和 `ai-review:execution-check` 已落地；后续 RAG snapshot、公开 API、Server Action、UI 保存、
   话术/任务下游真实创建或评测必须先读取并更新 `docs/contracts/ai-review-run.md`。
...
9. `talk-track-asset` 契约草案已建立，且本地-only 资产、版本、场景、区块、来源引用、
   AI 候选、审核发布和复用反馈 repository 已部分实现；后续浏览器保存、公开 API、
...
10. `next-session-task` 契约草案已建立，且本地-only 任务、来源证据、负责人、检查项、
    ...
    已部分实现；后续浏览器保存、公开 API、Server Action、AI 复盘下游任务候选、
...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    ...
    来源/审核/发布门禁、直播场次 repository 基础、知识生命周期 repository 基础、AI 复盘 run
    repository 基础、话术资产 repository 基础、下场任务 repository 基础、DeepSeek provider gate、
    AI 复盘 generation orchestrator 基础或 AI 复盘 execution service 基础；更合理的候选是阶段 2
    provider/login 选择、公开登录路由、middleware/team switching、产品库 UI 保存入口/编辑/API 扩展，
    或在 provider gate 之后推进 AI 复盘 MVP 的公开触发、保存、评测或 RAG 前置切片。
