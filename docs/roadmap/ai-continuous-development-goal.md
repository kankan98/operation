...
- V0 usable trial workflow 已完成 demo-data 和试用证据收口：`/` 和 `/trial` 在试用会话 ready 后汇总六个已实现工作面的
  ...
  当前验收包波次正在把该阶段判断进一步收口为内部 V0 发布结论、证据强度、阻断项、下一步动作和
  生产门禁说明，帮助团队判断是否扩大内部试用；
...
- 内部可试用 V0：目标是让运营或评估人员可以从 `/trial` 进入演示团队，按“场次 → 球拍 →
  ...
  V0.9 试用就绪 cockpit 收口估算，当前约 97%；本轮 V0 验收包归档、部署和公网 smoke
  完成后预计约 98%-99%。
...
来源信任问题和下游动作断点；AI 质量卡点先通过 `/ai-review` 修复优先级闭环收口，再决定是否进入
生产 AI 发布、RAG、正式评测或复核队列。
