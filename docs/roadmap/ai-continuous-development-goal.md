...
- 本地-only auth cookie runtime：server-only session cookie issue/clear header、request cookie
  resolver、logout invalidation、脱敏和本地 `auth:cookie-check` 回滚式验证；cookie helper 本身仍不创建登录
  provider、middleware、公开登录路由或公开保存流程。
...
- 本地-only 球拍产品库 API runtime：`GET /api/rackets/products` 和
  `POST /api/rackets/products` 通过现有 auth cookie/session runtime、显式 tenant/team scope、
  CSRF mutation header、repository business rules、no-store 安全响应和本地 `rackets:route-check`
  回滚式验证，提供首个受保护业务 Route Handler create/list 边界；仍不创建登录 provider、UI
  表单、Server Action、产品编辑、来源/审核/发布 API 或生产保存流程。
...
- 本地-only 知识生命周期 API runtime：`GET /api/knowledge/sources`、
  ...
  冲突处理和发布版本边界；仍不创建登录 provider、UI 保存流程、Server Action、公开来源抓取、
...
- 本地-only operator V0 session workflow：`POST /api/auth/operator-v0-session` 在显式启用和
  ...
  加载 scoped 场次、创建草稿、保存摘要/问题/异议并提交到 review-ready。它仍不是生产登录
  provider，且 `Secure` cookie 规格意味着 HTTP 公网 IP 预览不应被当成完整 authenticated browser 环境。
...
- 登录 provider、middleware、公开登录路由、邀请、团队管理 UI 和生产认证服务。
- 面向用户的登录后完整 CRUD、Server Action、生产数据库 provider、连接池、备份和恢复；
  ...
  `/sessions` 具备 operator V0 浏览器保存闭环，AI 复盘也具备 local-only 受保护 API runtime；
  其他业务域仍没有浏览器可操作保存流程。
- AI 复盘 UI 保存、Server Action、RAG、Q&A 模型调用、Web discovery、公开审核队列 UI；当前已有
  server-only DeepSeek provider adapter、AI 复盘 generation orchestrator、execution service 和受保护
  API runtime 的 fake-provider 本地验证。
...
1. 球拍产品库已进入本地受保护 API 准备阶段，产品、别名、来源、审核和发布门禁 repository
   已落地，且 create/list 已有 local-only 受保护 Route Handler；下一步避免重复，应转向
   UI 保存入口、产品编辑、来源/审核/发布 API、Server Action wrapper 或版本化 snapshot 前置契约。
2. 直播场次草稿、长文本、问题异议和提交 readiness 已进入本地 repository 与受保护 API runtime
   验证阶段；下一步若做公开保存流程，应基于已有 Route Handler 补浏览器表单状态、薄 Server Action
   或 fetch wrapper、真实登录会话和冲突恢复体验。
3. 知识生命周期来源、claim、团队笔记、审核队列、审核决策、发布版本和冲突处理已进入本地
   repository 与受保护 API runtime 验证阶段；下一步若开放浏览器保存、刷新任务、web discovery、
   AI/RAG grounding、反馈学习或公开审核 UI，
   必须先更新 `docs/contracts/knowledge-lifecycle.md` 和对应 OpenSpec。
4. AI 复盘输入快照、知识快照、输出、失败状态、人工审核、server-only generation orchestrator、
   server-only execution service 和受保护 API runtime 已进入本地验证阶段；下一步若接 RAG snapshot、
   浏览器保存、Server Action、队列、生产 provider 发布、真实 UI 触发或自动下游创建，
   必须先更新 `docs/contracts/ai-review-run.md` 并创建对应 OpenSpec。
5. 话术资产和下场任务闭环；话术资产和下场任务均已具备本地 repository 与受保护 API runtime
   验证，后续公开保存、AI 下游候选或团队看板仍需真实认证、浏览器 UI 和 Server Action/fetch 边界。
...
1. `racket-product-library` 契约草案已建立，且本地-only 产品、别名、来源、审核决策、发布门禁
   repository 和产品 create/list 受保护 Route Handler 已部分实现；后续产品库编辑、来源/审核/发布
   API、公开来源导入/发现、Server Action、UI 保存、AI/RAG grounding 或版本化 snapshot 必须先读取并更新
...
3. `knowledge-lifecycle` 契约草案已建立，且本地-only 来源登记、claim、团队笔记、审核队列、
   审核决策、冲突处理、发布版本和 readiness repository/API runtime 已部分实现；后续浏览器保存、
   刷新任务、web discovery、RAG snapshot 或 Q&A grounding 必须先读取并更新
...
4. `ai-review-run` 契约草案已建立，且本地-only run、输入快照、知识快照、prompt 版本元数据、
   ...
   受保护 AI 复盘 API runtime、`ai-provider:check`、`ai-review:generation-check`、
   `ai-review:execution-check` 和 `ai-review:route-check` 已落地；后续 RAG snapshot、Server Action、UI 保存、
   话术/任务下游真实创建、队列、生产发布或评测必须先读取并更新 `docs/contracts/ai-review-run.md`。
...
6. `auth-team-tenant` 契约草案已建立，且本地-only 授权守卫基础、app-owned session runtime、
   server-only cookie/request runtime 和 session/logout auth route runtime 已部分实现；后续认证
   provider 选择、公开登录路由、middleware、团队、租户、角色、
...
9. `talk-track-asset` 契约草案已建立，且本地-only 资产、版本、场景、区块、来源引用、
   AI 候选、审核发布和复用反馈 repository/API runtime 已部分实现；后续浏览器保存、
   Server Action、AI 复盘下游候选、Q&A/RAG grounding、复用反馈或短视频 hook 复用必须先读取并更新
   `docs/contracts/talk-track-asset.md`。
10. `next-session-task` 契约草案已建立，且本地-only 任务、来源证据、负责人、检查项、
    依赖、审核结果、反馈信号、状态流转、重复检测、敏感来源阻断和 readiness repository/API runtime
    已部分实现；后续浏览器保存、Server Action、AI 复盘下游任务候选、
    通知、日历、导出或团队回看必须先读取并更新 `docs/contracts/next-session-task.md`。
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    产品/别名 repository 基础、产品库 create/list Route Handler、产品库
    来源/审核/发布门禁、直播场次 repository/API/browser V0 基础、知识生命周期 repository/API runtime 基础、AI 复盘 run
    repository 基础、话术资产 repository/API runtime 基础、下场任务 repository/API runtime 基础、DeepSeek provider gate、
    AI 复盘 generation orchestrator 基础、AI 复盘 execution service 基础或 AI 复盘受保护 API runtime 基础；更合理的候选是阶段 2
    provider/login 选择、公开登录路由、middleware/team switching、产品库 UI 保存入口/编辑/API 扩展，
    或把 `/sessions` 已提交场次接到 AI 复盘 UI 保存/执行入口、评测、RAG 前置切片或队列设计。
    Q&A runtime、反馈学习和 web discovery 仍按技术实施阶段路线后置推进。
