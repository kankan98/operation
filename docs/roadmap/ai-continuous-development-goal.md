...
- 面向用户的 API、Server Action、受保护 CRUD、生产数据库 provider、连接池、备份和恢复；
  当前产品库、直播场次、知识生命周期、AI 复盘 run、话术资产和下场任务持久化仅限本地 repository 验证，没有公开保存流程。
...
1. 球拍产品库已进入本地持久化准备阶段，产品、别名、来源、审核和发布门禁 repository 已落地；
   下一步避免重复，应转向编辑、API/Server Action/UI 保存或版本化 snapshot 前置契约。
...
1. `racket-product-library` 契约草案已建立，且本地-only 产品、别名、来源、审核决策和发布门禁
   repository 已部分实现；后续产品库编辑、公开来源导入/发现、API、Server Action、UI 保存、
   AI/RAG grounding 或版本化 snapshot 必须先读取并更新 `docs/contracts/racket-product-library.md`。
...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    ...
    provider/login 选择、公开登录路由、middleware/team switching、首个受保护 Route Handler / Server Action，或在 provider
    gate 之后推进 AI 复盘 MVP 的公开触发、保存、评测或 RAG 前置切片。
