...
- 面向用户的登录后 API、Server Action、完整受保护 CRUD、生产数据库 provider、连接池、备份和恢复；
  当前只有产品库 create/list 具备 local-only 受保护 Route Handler 验证，其余直播场次、知识生命周期、
  AI 复盘 run、话术资产和下场任务持久化仍仅限本地 repository 验证，没有公开保存流程。
...
2. 直播场次草稿、长文本、问题异议和提交 readiness 已进入本地 repository 验证阶段；下一步若做
   公开保存流程，必须先补真实认证/Route Handler/Server Action 边界。
3. 知识生命周期来源、claim、团队笔记、审核、发布版本和冲突阻断已进入本地 repository 验证阶段；
   下一步若开放浏览器保存、公开 API、刷新任务、web discovery、AI/RAG grounding 或反馈学习，
...
2. `session-capture` 契约草案已建立，且本地-only 场次、主播职责、商品顺序、笔记、
   客户问题、购买异议、草稿 autosave、提交 readiness repository 已部分实现；后续浏览器保存、
   Route Handler、Server Action、转录导入、AI 复盘输入或外部平台同步必须先读取并更新
   `docs/contracts/session-capture.md`。
3. `knowledge-lifecycle` 契约草案已建立，且本地-only 来源登记、claim、团队笔记、审核决策、
   发布版本、冲突阻断和 readiness repository 已部分实现；后续浏览器保存、公开 API、
