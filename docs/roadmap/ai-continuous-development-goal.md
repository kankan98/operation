...
- 本地-only auth cookie runtime：server-only session cookie issue/clear header、request cookie
  resolver、logout invalidation、脱敏和本地 `auth:cookie-check` 回滚式验证；仍不创建登录
  provider、middleware、公开登录/登出路由或公开保存流程。
...
- 登录 provider、middleware、公开登录/登出路由、邀请、团队管理 UI 和生产认证服务。
...
6. `auth-team-tenant` 契约草案已建立，且本地-only 授权守卫基础、app-owned session runtime
   和 server-only cookie/request runtime 已部分实现；后续认证 provider 选择、公开登录/登出路由、
   middleware、团队、租户、角色、
...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、
    ...
    provider/login 选择、公开登录/登出路由、首个受保护 Route Handler / Server Action，或在 provider
