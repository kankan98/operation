...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    ...
    `/sessions` 到 `/ai-review` 再到 `/talk-tracks` / `/next-actions` 的 V0 浏览器闭环或 public trial route gate；更合理的候选是阶段 2
    provider/login 选择、team switching、产品库编辑、知识库刷新/版本回滚、AI 复盘评测、RAG 前置切片或队列设计。
