...
- V0 usable trial workflow 已完成 demo-data 和试用证据收口：`/` 和 `/trial` 在试用会话 ready 后汇总六个已实现工作面的
  ...
  敏感数据治理、AI/RAG 评测和观测脱敏拆成可扫读的阻断项与下一波建议。它只用于内部/演示试用导航，
...
- 生产可用版：需要生产登录/邀请/团队管理、HTTPS 域名、备份恢复、真实敏感数据治理、生产
  ...
  同一个百分比口径，后续按 V1/V2 分阶段推进。当前 V1 门禁规划已开始把生产访问、HTTPS、
  备份、敏感数据、AI/RAG 评测和观测脱敏拆成可验证前置项，下一轮优先推进生产访问与 HTTPS 门禁。
