...
- 面向用户的登录后完整 CRUD、Server Action、生产数据库 provider、连接池、备份和恢复；
  ...
  AI 复盘也具备 local-only 受保护 API runtime，但仍没有浏览器可操作保存流程。
...
11. 下一步不应再重复做数据基础、授权守卫基础、auth session runtime、auth cookie runtime、auth route runtime、
    ...
    来源/审核/发布门禁、直播场次 repository 基础、知识生命周期 repository/API runtime 基础、AI 复盘 run
    ...
    或在 AI 复盘 API runtime 之后推进 UI 保存、Server Action wrapper、评测、RAG 前置切片或队列设计。
